...
def rot2quaternion(rot_mat, eps=1e-6):
    ...
    if not torch.is_tensor(rot_mat):
        raise TypeError("Input type is not a torch.Tensor. Got {}".format(type(rot_mat)))

    if len(rot_mat.shape) > 3:
        raise ValueError("Input size must be a three dimensional tensor. Got {}".format(rot_mat.shape))
