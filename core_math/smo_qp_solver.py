...
class SMO_QP:
    def __init__(self, compression_ratio=0.1, num_batched_pairs=100, num_iters=50000, debug=False):
...
@torch.no_grad()
def solve_qp(dist_scores, kernel, compression_ratio=0.1, num_iters=10000, num_batched_pairs=100, debug=False):
    ...
    for batch_idx in range(0, num_iters, num_batched_pairs):
        ...
        for pair in pairs.unbind(0):
            ...
            alpha[i] = compute_probability(dist_scores, kernel, i, j, alpha, compression_ratio)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        kernel_sqrt, _ = sqrt_newton_schulz_autograd(kernel.unsqueeze(0), numIters=15, dtype=kernel.dtype)
