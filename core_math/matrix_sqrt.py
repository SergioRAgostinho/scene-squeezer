...
def sqrt_svd_lyap(A, dldz, dtype):
    ...
    for i in range(batchSize):
        ...
        IU = U.t()
        X = -U.mm(((IU.mm(dldz[i, :, :].data)).mm(IU.t())) / (S + S.t())).mm(U.t())
        dlda[i, :, :] = X
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tau", type=float, default=1.0, metavar="N", help="conditioning by adding to the diagonal (default: 1.0)"
    )
    parser.add_argument(
        "--num-iters", type=int, default=10, metavar="N", help="number of schulz iterations (default: 5)"
    )
    parser.add_argument("--dim", type=int, default=64, metavar="N", help="size of the covariance matrix (default: 64)")
    ...
    A = Variable(
        create_symm_matrix(batchSize=args.batch_size, dim=args.dim, numPts=args.pts, tau=args.tau, dtype=d),
        requires_grad=True,
    )
