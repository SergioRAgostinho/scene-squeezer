...
class PeriodicCheckpoint(ModelCheckpoint):
    ...
    def __save__(self, trainer: pl.Trainer, pl_module: pl.LightningModule, epoch_end_flag=False, stop_flag=False):
        ...
        LightningLogger.add_text(
            trainer.logger.experiment, text="ckpt saved: %s" % str(ckpt_path), tag="ckpt", step=pl_module.global_step
        )
        if self.verbose:
            notice_msg(
                "checkpoint saved to %s [STEP: %d | EPOCH: %d]"
                % (ckpt_path, pl_module.global_step, pl_module.current_epoch),
                obj=self,
            )
    ...
    def logger_factory(self, logger_name):
        if logger_name == "csv":
            ...
            return pl_loggers.TensorBoardLogger(
                save_dir=self.exp_dir, name=self.params.NAME_TAG, default_hp_metric=False
            )
    ...
    def report(self):
        ...
        for _, logger in self.loggers.items():
            if isinstance(logger, pl_loggers.CometLogger):
                msg(
                    "api_key: %s, project_name: %s" % (str(logger.api_key), str(logger._project_name)),
                    obj="CometLogger",
                )
            elif isinstance(logger, pl_loggers.TensorBoardLogger):
                msg("saved_dir: %s" % (str(logger.save_dir)), obj="TensorBoardLogger")
            elif isinstance(logger, pl_loggers.CSVLogger):
                msg("saved_dir: %s" % (str(logger.save_dir)), obj="CSVLogger")
