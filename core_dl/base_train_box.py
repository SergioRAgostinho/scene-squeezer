...
class BaseTrainBox:
    ...
    def __init__(self, train_params: TrainParameters, load_optimizer=False):
        ...
        if isinstance(self.ckpt_path_dict, str) and not file_not_exists(
            self.ckpt_path_dict, self, raise_exception=False
        ):
            ...
            for ckpt_key in self.ckpt_path_dict:
                if file_not_exists(self.ckpt_path_dict[ckpt_key], self, raise_exception=False):
                    self.model_state_dict[ckpt_key] = torch.load(
                        self.ckpt_path_dict[ckpt_key], map_location=torch.device("cpu")
                    )
        ...
        if self.load_optimizer is True:
            if (
                ...
                and self.model_state_dict is not None
                and "optimizer" in self.model_state_dict.keys()
            ):
        ...
        if self.verbose_mode and self.logger is not None:
            ...
            if logger is not None:
                ...
                logger.add_text(tag="Description", text_string=name_str.replace("\n", "  \n"), global_step=0)

                dir_str = "LOG_DIR: %s\nDEBUG_DUMP_DIR: %s" % (
                    self.logger.log_base_dir,
                    str(self.train_params.DEBUG_OUTPUT_DIR),
                )
                logger.add_text(tag="Directory", text_string=dir_str.replace("\n", "  \n"), global_step=0)

                logger.add_text(
                    tag="Parameters", text_string=str(self.train_params).replace("\n", "  \n"), global_step=0
                )
    ...
    def _set_logger(self, workspace_dir: str, comment_msg: str):
        ...
        if workspace_dir is not None:
            ...
            if isinstance(self.train_params.CKPT_DICT, str) and self.train_params.LOG_CONTINUE_STEP > 0:
                self.logger = Logger(
                    base_dir=self.train_params.LOG_CONTINUE_DIR,
    ...
    def _train_feed(
        self, train_sample: list, cur_train_epoch: int, cur_train_itr: int, logger=None, eval_flag=False
    ) -> dict:
        """
            [Override function]
    ...
    def save_checkpoint(self, epoch=None, itr=None, path=None):
        ...
        if self.logger is not None:
            ...
            if self.logger is not None:
                self.logger.log(
                    {"Iteration": itr + 1, "Epoch": epoch, "Event": "Check point saved to %s" % checkpoint_file_name}
                )
            ...
            if tf_logger is not None:
                tf_logger.add_text(
                    tag="Checkpoints", text_string="Save checkpoint to " + checkpoint_file_name, global_step=(itr + 1)
                )
