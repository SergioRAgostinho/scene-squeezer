...
def forward_dual_matcher(self, input: list, return_type="train"):
    ...
    with torch.cuda.device(self.device_of(self.dual_matcher)) as _:
        ...
        for q_i in range(min(q_metas.num_frames(), 3)):
            ...
            r2q_matches = torch.cat(
                [torch.arange(0, r_xyz.shape[0]).int().unsqueeze(-1), r2q_matches.int().unsqueeze(-1)], dim=-1
            ).long()
            ...
            inlier_ratio = measure_r2q_inlier_ratio(
                q_metas.K[q_i], q_metas.Tcws[q_i], q_pt2d.uv[q_i], r_xyz, r2q_matches_valids
            )
        ...
        if return_type == "debug":
            if len(r2q_avg_inliers_ratios) == 0:
                inlier_ratios_avg = torch.tensor(0.0, dtype=float)
            else:
