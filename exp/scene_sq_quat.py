...
def forward_soft_quant(self, input: list, return_type="train"):
    ...
    with torch.cuda.device(self.device_of(self.pt_sel)) as _:
        ...
        r_ori_feats = r_ori_feats.to(cur_dev)
        r_xyz = r_xyz.to(cur_dev)

        if self.is_reduce_dim:
            N, C, M = r_ori_feats.shape
            # r_f_norms = torch.norm(r_ori_feats, dim=1)
            r_feats = self.quantizater.encode(r_ori_feats)
            r_feats = self.quantizater.decode(r_feats)
    ...
    with torch.cuda.device(self.device_of(self.dual_matcher)) as _:
        ...
        for q_i in range(q_metas.num_frames()):
            gt_q2r = gen_gt_corres(
                ...
                q_metas.dims[q_i],
                r_xyz,
                reproj_dist_thres=8.0,
                only_valid_pairs=True,
            )
            if gt_q2r is None:
                gt_r2qs.append(None)
                continue
            gt_r2q = inverse_matches(gt_q2r.cpu())
            full_gt_r2q = torch.cat(
                [
                    torch.arange(0, r_xyz.shape[0]).unsqueeze(-1).int(),
                    -1 * torch.ones(r_xyz.shape[0]).unsqueeze(-1).int(),
                ],
                dim=-1,
            )
        ...
        # l2 loss
        r_feat_l2 = torch.norm(r_feats - r_ori_feats, dim=1)
        r_feat_l2_mean = r_feat_l2.mean()
        if self.training:
            r_feat_l2_mean.backward(retain_graph=True)
        ...
        #     total_loss = 0.1 * (r2q_mloss + 0.1 * r2q_inv_mloss)
    ...
    return (
        {
            "feat_recover_loss": r_feat_l2_mean.item(),
            "dual_vis_mloss": sum(r2q_visible_losses) / len(r2q_visible_losses),
