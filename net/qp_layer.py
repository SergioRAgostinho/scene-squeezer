...
def get_qp_layer(num_features, compression_ratio=0.7, distinctiveness_weight=0.5):
...
class QPLayer(nn.Module):
    ...
    def _set_qp_layer(self) -> None:
        ...
        self.qp_sqrtm_iters = from_meta(self.args, "qp_sqrtm_iters", default=10)
        ...
        self.qp_layer = get_qp_layer(
            self.qp_max_input_keypoints, self.qp_compression_ratio, self.qp_distinctiveness_weight
        )
    ...
    def forward(self, dist_score: torch.Tensor, rbf_kernel: torch.Tensor) -> torch.Tensor:
        ...
        rbf_kernel_sqrt, _ = sqrt_newton_schulz_autograd(
            rbf_kernel_subset.unsqueeze(0), numIters=self.qp_sqrtm_iters, dtype=rbf_kernel_subset.dtype
        )
