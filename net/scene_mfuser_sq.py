...
from core_io.print_msg import err_msg
from exp.scene_sq_utils import move_to_origin, normalize_3dpts, normalize_kpts_pos
...
class SceneSqueezerWithTestQueries(nn.Module):
    ...
    def forward(self, vr_meta: ClipMeta, vr_pt2d: Pt2dObs, r_meta: ClipMeta, r_pt2d: Pt2dObs, r_pt3d: Pt3dObs):
        ...
        with torch.no_grad():
            ...
            r_aggr_feats = rearrange(r_aggr_feats, "m c -> () c m")

        if r_pt3d.xyz.shape[0] < 128:
            return (None, None), (r_pt3d.xyz, r_aggr_feats), None
        ...
        return (log_var.view(1, N), k_feats.view(1, N, -1)), (r_pt3d.xyz, r_aggr_feats), {"in_feats": in_feats}
